  ...
  return (
    ...
    >
      <div className="flex flex-col h-full items-center">
        ...
        {/* Description - Hidden on smallest screens, one line on larger */}
        <p className="hidden min-[360px]:block text-[10px] sm:text-xs text-gh-fg-muted mb-2 sm:mb-3 line-clamp-1 text-center leading-relaxed w-full px-1">
          {translatedDescription}
        </p>

        {/* Categories - Compact badges, adjusted for mobile */}
        <div className="flex flex-wrap gap-0.5 sm:gap-1 justify-center mt-auto">
          {/* Custom badge if tool is custom */}
          {isCustom && (
            <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] leading-tight rounded bg-purple-500/10 text-purple-400 border border-purple-500/20 font-medium">
              {t('card.custom')}
            </span>
          )}
          {/* Regular categories */}
          {tool.category.filter(cat => cat !== 'Ajout personnel').slice(0, isCustom ? 1 : 2).map((cat, index) => (
            <span
              key={index}
              className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] leading-tight rounded bg-gh-accent-subtle text-gh-accent-fg border border-gh-border-muted font-medium"
            >
              {t(`categories.${cat}`)}
            </span>
          ))}
          {tool.category.filter(cat => cat !== 'Ajout personnel').length > (isCustom ? 1 : 2) && (
            <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] leading-tight text-gh-fg-subtle">
              +{tool.category.filter(cat => cat !== 'Ajout personnel').length - (isCustom ? 1 : 2)}
            </span>
          )}
        </div>
